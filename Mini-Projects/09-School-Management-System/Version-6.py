# Added a function to add subject-wise marks to a student
# Created 3 new validator funcitions
# Added a feature to show students report card

class Person:
    def __init__(self, name, age):
        self.name = name
        self.age = age

    def describe(self):
        print(f"\nName: {self.name}")
        print(f"Age = {self.age}")

class Student(Person):
    def __init__(self, name, age, roll_number, gpa):
        super().__init__(name, age)
        self.roll_number = roll_number
        self.gpa = gpa
        self.marks = {} #Now every student have their own marks history
    
    def describe_student(self):
        super().describe()
        print(f"Roll Number = {self.roll_number}")
        print(f"GPA = {self.gpa}")

    def __str__(self):
        return f"{self.name} | Roll No: {self.roll_number} | GPA: {self.gpa}"

class Teacher(Person):
    def __init__(self, name, age, subject, teacher_id):
        super().__init__(name, age)
        self.subject = subject
        self.teacher_id = teacher_id
    
    def describe_teacher(self):
        super().describe()
        print(f"Subject = {self.subject}")
        print(f"Id = {self.teacher_id}")

    def __str__(self):
        return f"{self.name} | Subject: {self.subject}"

# ===============
# Storage  
# ===============
students_list = []
teachers_list = []

# ========================
# Student Features 
# ========================
def add_students():
    print("\n=== Add Student===")
    name = get_name("Enter name: ")
    age = get_int("Enter age: ")
    roll_number = get_int("Enter Roll no: ")
    for student in students_list:
        if student.roll_number == roll_number:
            print("Roll number already exists.")
            return
    gpa = get_gpa("Enter GPA: ")

    student = Student(name, age, roll_number, gpa)

    students_list.append(student)

    print(f"\n{student.name} added successfully!")

def show_students():
    print("\n=== Student List ===")
    if not students_list:
        print("No students added yet.")
        return

    for x in students_list:
        x.describe_student()

def remove_students():
    print("\n=== Remove Student ===")

    if not students_list:
        print("No students added yet.")
        return

    roll_number = get_int("Enter roll number to remove: ")

    removed = False

    for student in students_list:

        if student.roll_number == roll_number:

            students_list.remove(student)

            print(f"\n==== {student.name} removed successfully. ====")
            removed = True
            break

    if not removed:
        print("No student found with that roll number.")

def search_students():
    print("\n=== Search Student ===")

    if not students_list:
        print("No students added yet.")
        return

    roll_number = get_int("Enter roll number to search: ")

    found = False

    for student in students_list:

        if student.roll_number == roll_number:

            print("\n==== Student Found ====")
            student.describe_student()

            found = True
            break

    if not found:
        print("No student found with that roll number.")

def update_gpa():
    print("\n=== Update GPA ===")

    if not students_list:
        print("No students added yet.")
        return  
    
    roll_number = get_int("Enter roll number of student whose GPA is to be changed: ")
    changed_GPA = get_float("Enter the new GPA: ")
    
    changed = False

    for student in students_list:

        if student.roll_number == roll_number:

            old_gpa = student.gpa
            student.gpa = changed_GPA

            print(f"\n==== GPA changed from {old_gpa} to {student.gpa}====")

            changed = True
            break

    if not changed:
        print("No student found with that roll number.")

def add_marks():
    print("\n=== Add Marks ===")

    if not students_list:
        print("No students added yet.")
        return
    
    roll_number = get_int("Enter roll number of student to add marks: ")
    added = False

    for student in students_list:
            
            if student.roll_number == roll_number:

                while True:

                    subject_to_add = get_subject("Enter the subject: ")
                    marks_to_add = get_marks("Enter marks: ")
                    student.marks[subject_to_add] = marks_to_add
                    print("=== Subject and Marks added===")

                    choice = input("Do you want to add more subject (y/n)").lower()
                    if choice == 'y':
                        continue
                    else:
                        break
                
                added = True
                break
    if not added:
        print("No student found with that roll number.")   

def show_report_card():

    print("\n=== Report Card ===")

    if not students_list:
        print("No students added yet.")
        return

    roll_number = get_int("Enter roll number of student to see report card: ")

    search = False

    for student in students_list:

        if student.roll_number == roll_number:

            print(f"\nName: {student.name}")
            print(f"Roll Number: {student.roll_number}")
            print(f"GPA: {student.gpa}")

            if not student.marks:
                print("No marks added yet.")
            else:
                print("\n--- Marks ---")

                for subject, marks in student.marks.items():
                    print(f"{subject}: {marks}")
                
                average = sum(student.marks.values()) / len(student.marks)
                print(f"Average = {average:.2f}")

                if average >= 90:
                    grade = "A"

                elif average >= 80:
                    grade = "B"

                elif average >= 70:
                    grade = "C"

                elif average >= 60:
                    grade = "D"

                else:
                    grade = "F"
                    
                print(f"Grade = {grade}")

            search = True
            break

    if not search:
        print("No student found with that roll number.")
# ========================
# Teacher Features
# ========================
def add_teachers():
    print("\n=== Add Teacher===")
    name = get_name("Enter name: ")
    age = get_int("Enter age: ")
    subject = get_subject("Enter subject: ")
    teacher_id = get_int("Enter teacher Id: ")
    for teacher in teachers_list:
        if teacher.teacher_id == teacher_id:
            print("Teacher Id already exists.")
            return

    teacher = Teacher(name,age, subject, teacher_id)

    teachers_list.append(teacher)

    print(f"\n{teacher.name} added successfully!")

def show_teachers():
    print("\n=== Teacher List ===")
    if not teachers_list:
        print("No teachers added yet.")
        return

    for x in teachers_list:
        x.describe_teacher()

def remove_teachers():
    print("\n=== Remove Teacher ===")

    if not teachers_list:
        print("No teachers added yet.")
        return

    teacher_id = get_int("Enter teacher Id to remove: ")

    removed = False

    for teacher in teachers_list:

        if teacher.teacher_id == teacher_id:

            teachers_list.remove(teacher)

            print(f"\n==== {teacher.name} removed successfully. ====")
            removed = True
            break

    if not removed:
        print("No teacher found with that Id.")

def search_teachers():
    print("\n=== Search Teacher ===")

    if not teachers_list:
        print("No teachers added yet.")
        return

    teacher_id = get_int("Enter teacher Id to search: ")

    found = False

    for teacher in teachers_list:

        if teacher.teacher_id == teacher_id:

            print("\n==== Teacher Found ====")
            teacher.describe_teacher()

            found = True
            break

    if not found:
        print("No teacher found with that Id.")

def update_subject():
    print("\n=== Update Subject ===")

    if not teachers_list:
        print("No teacher added yet.")
        return  
    
    teacher_id = get_int("Enter Id of teacher whose subject is to be changed: ")
    changed_subject = get_subject("Enter the new subject: ")
    
    changed = False

    for teacher in teachers_list:

        if teacher.teacher_id == teacher_id:

            old_subject = teacher.subject
            teacher.subject = changed_subject

            print(f"\n==== Subject changed from {old_subject} to {teacher.subject}====")

            changed = True
            break

    if not changed:
        print("No teacher found with that Id.")

# =================
# Helper function
# =================
def get_name(prompt):
    while True:
        value = input(prompt).strip()

        if not value:
            print("Name cannot be empty.")
            continue

        if not value.replace(" ", "").isalpha():
            print("Name should contain only letters.")
            continue

        return value.title()

def get_int(prompt):
    while True:
        try:
            value = int(input(prompt))
            if value > 0:
                return value
            print("Enter a positive number.")
        except ValueError:
            print("Invalid number.")

def get_float(prompt):
    while True:
        try:
            value = float(input(prompt))
            if value > 0:
                return value
            print("Enter a positive number.")
        except ValueError:
            print("Invalid number.")

def get_choice_inner(prompt):
    while True:
        try:
            value = int(input(prompt))
            if 1 <= value <=8:
                return value
            print("Enter a number between 1-8.")
        except ValueError:
            print("Invalid number.")

def get_choice_outer(prompt):
    while True:
        try:
            value = int(input(prompt))
            if 1 <= value <=3:
                return value
            print("Enter a number between 1-3.")
        except ValueError:
            print("Invalid number.")

def get_gpa(prompt):

    while True:

        try:
            value = float(input(prompt))

            if 0 <= value <= 10:
                return value

            print("GPA must be between 0 and 10.")

        except ValueError:
            print("Invalid GPA.")

def get_marks(prompt):

    while True:

        try:
            value = int(input(prompt))

            if 0 <= value <= 100:
                return value

            print("Marks must be between 0 and 100.")

        except ValueError:
            print("Invalid marks.")

def get_subject(prompt):

    while True:

        value = input(prompt).strip()

        if not value:
            print("Subject cannot be empty.")
            continue

        return value.title()

# ================
# Main Program
# ================
def main():

    while True:

        print("\n==== School Management System ====")
        print("---> Main Menu ")
        print("1. Manage Students")
        print("2. Manage Teachers")
        print("3. Exit")

        choice = get_choice_outer("\nEnter your choice: ")
        # =========================
        # Student Menu
        # =========================

        if choice == 1:

            while True:

                print("\n===== Student Menu =====")
                print("1. Add Student")
                print("2. Remove Student")
                print("3. Search Student")
                print("4. Show Students List")
                print("5. Update Student GPA")
                print("6. Add Subjects and Marks")
                print("7. Show Report Card")
                print("8. Return to Main Menu")

                choice1 = get_choice_inner("\nEnter your choice: ")

                if choice1 == 1:
                    add_students()

                elif choice1 == 2:
                    remove_students()

                elif choice1 == 3:
                    search_students()

                elif choice1 == 4:
                    show_students()

                elif choice1 == 5:
                    update_gpa()

                elif choice1 == 6:
                    add_marks()

                elif choice1 == 7:
                    show_report_card()

                elif choice1 == 8:
                    print("\nReturning to Main Menu...")
                    break

                else:
                    print("Invalid choice!")
        # =========================
        # Teacher Menu
        # =========================

        elif choice == 2:

            while True:

                print("\n===== Teacher Menu =====")
                print("1. Add Teacher")
                print("2. Remove Teacher")
                print("3. Search Teacher")
                print("4. Show Teachers List")
                print("5. Update Teacher Subject")
                print("6. Return to Main Menu")

                choice2 = get_choice_inner("\nEnter your choice: ")

                if choice2 == 1:
                    add_teachers()

                elif choice2 == 2:
                    remove_teachers()

                elif choice2 == 3:
                    search_teachers()

                elif choice2 == 4:
                    show_teachers()

                elif choice2 == 5:
                    update_subject()

                elif choice2 == 6:
                    print("\nReturning to Main Menu...")
                    break

                else:
                    print("Invalid choice!")
        # =========================
        # Exit
        # =========================
        elif choice == 3:
            print("\nGoodbye!")
            break

        else:
            print("Invalid choice!")
# =================           
# ===Run program===
# =================
if __name__ == "__main__":
    main()